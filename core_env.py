...
class IncidentOpsEnv(Environment):
    ...
    def step(self, action: Action) -> Observation:  # type: ignore[override]
        ...
        if done and grade:
            previous_sum = self._state.cumulative_reward - scalar
            final_step_reward = grade.total - previous_sum
            obs_ret.reward = float(round(final_step_reward, 4))
            self._state.cumulative_reward = grade.total
